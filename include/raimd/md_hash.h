#ifndef __rai_raimd__md_hash_h__
#define __rai_raimd__md_hash_h__
...
struct MergeCtx {
  ...
  void start( size_t start,  size_t sz,  size_t count,  size_t data_size,
    ...
    this->len        = sz_min( sz, count ); /* count of hash elems */
    this->left       = sz_min( data_size - start, this->len );
...
struct HashStorage : public ListStorage<UIntSig, UIntType> {
  ...
  void hash_delete( const ListHeader &hdr,  size_t pos ) {
    ...
    if ( pos == this->count ) /* delete the last item */
      return;
    sz    = this->get_size( hdr, 0, start, end );
    len   = sz_min( this->count + 1, sz ); /* already deleted list item */
    end   = hdr.data_offset( start, len );
  ...
  void get_hash_bits( const ListHeader &hdr,  uint64_t *bits ) const {
    ...
    if ( end >= start )
      ...
      j = sz_min( hdr.data_size() - start, len );
    for ( i = 1; i < j; i++ )
  ...
  int hverify( const ListHeader &hdr ) const {
    ...
    for ( size_t i = 1; i < len; i++ ) {
      ...
      uint32_t h = hash32( kv.key, kv.keylen, 0 );
...
struct HashData : public ListData {
  ...
  static const uint16_t hsh8_sig  = 0xf7e5U;
  static const uint32_t hsh16_sig = 0xddbe7ae5UL;
  static const uint64_t hsh32_sig = 0xa5f5ff85c9f6c3e5ULL;
  void init( size_t count,  size_t data_len ) {
    this->init_sig( count, data_len, hsh8_sig, hsh16_sig, hsh32_sig );
  }
...
#define HASH_CALL( GOTO ) \
  ...
  HashStatus hset( const void *key,  size_t keylen,
    ...
    HashPos pos( key, keylen );
    return this->hset( key, keylen, val, vallen, pos );
  }
  HashStatus hset( const void *key,  size_t keylen,
                   const void *val,  size_t vallen,  HashPos &pos ) {
  ...
  HashStatus hdel( const void *key,  size_t keylen ) {
    HashPos pos( key, keylen );
    return this->hdel( key, keylen, pos );
  }
...
struct HashMsg : public MDMsg {
  ...
  static bool is_hashmsg( void *bb,  size_t off,  size_t len,  uint32_t h );
